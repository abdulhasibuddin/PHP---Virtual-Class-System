...
//---------------------------------------------------------------------------
	...
	$fName = $lName = $eMail = $profession = $gender = $password = $conPasswrd = $captcha = "";
	$fNameErr = $lNameErr = $eMailErr = $profErr = $genderErr = $conPasswordErr = $captchaErr = "";
...
//---------------------------------------------------------------------------
	...
	//-----------------------------------------------------------------------
		$profession = secureInput($_POST["profession"]); //XSS vulnerability checking of input username
		if(strlen($profession) > 255) {
			$profErr = "Maximum 255 characters allowed!"; 
			$errFlag = 1;
		}

		if(!preg_match("/^[a-zA-Z0-9]{1,255}$/", $profession)) { //Regular expression comparison
			//If input profession is invalid::
			$profErr = "Only letters, numbers and underscore(_) allowed!";
			$errFlag = 1;
		}
		/*elseif($errFlag == 0) { //If input length is OK::
...
//---------------------------------------------------------------------------
	...
	if (($fName == "" || $lName == "" || $eMail == "" || $profession == "" || $gender == "" || $password == "" || $conPasswrd == "") || $errFlag == 1) { }
